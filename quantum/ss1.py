...
def compute_ss1_y3(qc, x, y, anc):
    ...
    qc.ccx(x[0], x[1], anc[0])
    qc.ccx(anc[0], x[3], y[3])
    qc.ccx(x[0], x[1], anc[0])  

def compute_ss1_y2(qc, x, y, anc):
    qc.x(y[2])                 
    qc.cx(x[1], y[2])           
    qc.cx(x[2], y[2])           
    qc.cx(x[3], y[2])           
...
compute_ss1_y3(qc, qr_in, qr_out, anc)
qc.barrier()
compute_ss1_y2(qc, qr_in, qr_out, anc)
qc.barrier()
compute_ss1_y1(qc, qr_in, qr_out, anc)
qc.barrier()
compute_ss1_y0(qc, qr_in, qr_out, anc)

qc.draw(output='mpl', filename='ss1.png', fold=30)
...
tt = [0x6, 0x4, 0x0, 0xd, 0x2, 0xb, 0xa, 0x3, 0x9, 0xc, 0xe, 0xf, 0x8, 0x7, 0x5, 0x1]
